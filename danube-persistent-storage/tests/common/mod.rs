...
use danube_persistent_storage::{BackendConfig, CloudStore, EtcdMetadata, LocalBackend, WalStorageFactory, ObjectDescriptor};
...
pub async fn create_test_factory() -> (WalStorageFactory, Arc<MemoryStore>) {
    ...
    let factory = WalStorageFactory::new(
        "/tmp/danube-integration-test".to_string(),
        cloud_store,
        metadata,
    );
...
pub async fn count_cloud_objects(memory_store: &MemoryStore, topic_path: &str) -> usize {
    ...
    let children = memory_store.get_childrens(&prefix).await.unwrap_or_default();
    children.into_iter().filter(|c| c != "cur" && !c.ends_with('/')).count()
}
...
pub async fn get_latest_object_descriptor(
    ...
    let mut objects: Vec<_> = children.into_iter().filter(|c| c != "cur" && !c.ends_with('/')).collect();
    objects.sort();
...
mod tests {
    ...
    async fn test_create_test_factory() {
        ...
        let storage = factory.for_topic("test/topic").await.expect("create storage");
        
        // Test basic append operation
        let msg = make_test_message(1, 1, "test/topic", 0, "test");
        storage.append_message("test/topic", msg).await.expect("append message");
        
        // Note: WalStorage doesn't expose current_offset directly, so we test via reader
        let mut reader = storage.create_reader("test/topic", StartPosition::Offset(0)).await.expect("create reader");
        let read_msg = reader.next().await.expect("read message").expect("message result");
    ...
    async fn test_count_cloud_objects() {
        ...
        let storage = factory.for_topic("test/count").await.expect("create storage");
        ...
        for i in 0..5 {
            ...
            storage.append_message("test/count", msg).await.expect("append message");
        }
