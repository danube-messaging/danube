...
use common::{create_test_factory, make_test_message, wait_for_condition, count_cloud_objects};
use danube_core::message::StartPosition;
...
async fn test_multiple_topics_isolated_storage() {
    ...
    for (i, topic) in topics.iter().enumerate() {
        let storage = &storages[*topic];
        let current_offset = storage.current_offset().await.expect("get offset");
        assert_eq!(current_offset, 10);
        
        let mut reader = storage.create_reader(StartPosition::Offset(0)).await.expect("create reader");
...
async fn test_multiple_topics_independent_uploaders() {
    ...
    for topic in &topics {
        ...
        assert!(object_count > 0, "Topic {} should have cloud objects", topic);
        
        // Verify objects contain correct topic data
        let prefix = format!("/danube/storage/topics/{}/objects", topic);
        let children = memory_store.get_childrens(&prefix).await.expect("get children");
        let objects: Vec<_> = children.into_iter().filter(|c| c != "cur" && !c.ends_with('/')).collect();
        
        assert!(!objects.is_empty(), "Topic {} should have object descriptors", topic);
    }
}
...
async fn test_multiple_topics_concurrent_operations() {
    ...
    let topics = vec!["concurrent/topic-1", "concurrent/topic-2", "concurrent/topic-3", "concurrent/topic-4"];
    ...
    for (i, topic) in topics.iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            let storage = factory_clone.for_topic(&topic_name).await.expect("create storage");
...
async fn test_multiple_topics_different_read_patterns() {
    ...
    let mut reader_a = storage_a.create_reader(StartPosition::Offset(0)).await.expect("create reader A");
    ...
    let mut reader_b = storage_b.create_reader(StartPosition::Offset(15)).await.expect("create reader B");
    ...
    let mut reader_c = storage_c.create_reader(StartPosition::Latest).await.expect("create reader C");
    ...
    tokio::spawn({
        ...
        async move {
            ...
            for j in 40..50u64 {
                let msg = make_test_message(j, 3, topics[2], j, &format!("pattern-2-{}", j));
                let _ = storage_c.append_message(topics[2], msg).await;
            }
        }
    });
...
async fn test_multiple_topics_factory_reuse() {
    ...
    let storage1 = factory.for_topic(topic_name).await.expect("create storage 1");
    let storage2 = factory.for_topic(topic_name).await.expect("create storage 2");
    let storage3 = factory.for_topic(topic_name).await.expect("create storage 3");
    ...
    for (i, storage) in [&storage1, &storage2, &storage3].iter().enumerate() {
        let current_offset = storage.current_offset().await.expect("get offset");
        assert_eq!(current_offset, 3, "Storage {} should see all messages", i + 1);
...
async fn test_multiple_topics_mixed_workloads() {
    ...
    let heavy_storage = factory.for_topic(heavy_topic).await.expect("create heavy storage");
    let light_storage = factory.for_topic(light_topic).await.expect("create light storage");
    let burst_storage = factory.for_topic(burst_topic).await.expect("create burst storage");
        ...
        tokio::spawn(async move {
            ...
            for burst in 0..3 {
                for i in 0..15u64 {
                    ...
                    let msg = make_test_message(offset, 3, burst_topic, offset, &format!("burst-{}-{}", burst, i));
                    storage
    ...
    let current_offset = heavy_storage.current_offset().await.expect("get heavy offset");
    assert_eq!(current_offset, 100);
    
    let mut heavy_reader = heavy_storage.create_reader(StartPosition::Offset(0)).await.expect("create heavy reader");
    ...
    let current_offset = light_storage.current_offset().await.expect("get light offset");
    assert_eq!(current_offset, 20);
    
    let mut light_reader = light_storage.create_reader(StartPosition::Offset(0)).await.expect("create light reader");
    ...
    let current_offset = burst_storage.current_offset().await.expect("get burst offset");
    assert_eq!(current_offset, 45);
    
    let mut burst_reader = burst_storage.create_reader(StartPosition::Offset(0)).await.expect("create burst reader");
    ...
    println!("Cloud objects - Heavy: {}, Light: {}, Burst: {}", 
