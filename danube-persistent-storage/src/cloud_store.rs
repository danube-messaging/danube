...
impl CloudStore {
    ...
    pub async fn open_streaming_writer(
        ...
        let writer = self
            ...
            .await
            .map_err(|e| PersistentStorageError::Other(format!("cloud writer_with {}: {}", key, e)))?;
        Ok(CloudWriter { inner: writer, bytes_written: 0 })
    }
    ...
    pub async fn open_ranged_reader(
        ...
        let reader = self
            ...
            .reader(&key)
            .await
            .map_err(|e| PersistentStorageError::Other(format!("cloud reader {}: {}", key, e)))?;
        ...
        Ok(CloudRangeReader { inner: reader, offset: start_byte, size })
    }
    ...
    pub async fn copy_object(
        ...
        self.op
            .copy(&from, &to)
            .await
            .map_err(|e| PersistentStorageError::Other(format!("cloud copy {} -> {}: {}", from, to, e)))
    }
...
impl CloudRangeReader {
    ...
    pub async fn read_chunk(&mut self, chunk_size: usize) -> Result<Vec<u8>, PersistentStorageError> {
