...
pub async fn list_namespaces_with_policies(
    ...
    for ns in unique_names.iter() {
        ...
        let req = danube_core::admin_proto::NamespaceRequest { name: ns.clone() };
        let topics_list = match state.client.list_namespace_topics(req).await {
            Ok(v) => v,
            Err(e) => {
                errors.push(format!("list_topics failed for {}: {}", ns, e));
                // continue with empty topics but still gather policies
                // Default empty response
                danube_core::admin_proto::TopicInfoListResponse { topics: Vec::new() }
            }
        };

        let topics: Vec<String> = topics_list.topics.into_iter().map(|t| t.name).collect();

        // get policies for this namespace
        let req = danube_core::admin_proto::NamespaceRequest { name: ns.clone() };
        let policies_resp = match state.client.get_namespace_policies(req).await {
            Ok(v) => v,
            Err(e) => {
